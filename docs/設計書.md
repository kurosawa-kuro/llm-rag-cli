...
### 4.13 graph.py — LangGraph ワークフロー定義
...
| make lint | 全 app/ モジュールの構文チェック（py_compile、15モジュール） |
...
`AppContainer` が重量級リソース（モデルロード・DB接続）を lazy property で管理する。初回アクセス時に遅延読み込みし、以降は同一インスタンスを再利用する。

**AppContainer の依存管理:**

| property | 管理対象 | 生成元ファクトリ |
|---|---|---|
| embeddings | HuggingFaceEmbeddings インスタンス | `embeddings.create_embeddings()` |
| vectorstore | PGVector インスタンス | `db.create_vectorstore(embeddings)` |
| reranker | CrossEncoderReranker インスタンス | `reranker.create_reranker()` |
| llm | LlamaCpp インスタンス | `llm.create_llm()` |
| prompt_builder | プロンプト構築関数 | `prompting.build_prompt` |
| retrieval_strategy | TwoStageRetrieval インスタンス | `retrieval.TwoStageRetrieval(...)` |

**シングルトン管理:**

| モジュール | グローバル変数 | 説明 |
|---|---|---|
| container.py | `_container` | AppContainer のシングルトン |
| graph.py | `_graph` | コンパイル済み StateGraph のシングルトン（container 未指定時のみ） |
...
|---|---|---|
| test_config.py | config.py | 25 |
| test_interfaces.py | interfaces.py | 5 |
| test_container.py | container.py | 21 |
| test_retrieval.py | retrieval.py | 6 |
| test_db.py | db.py | 6 |
| test_db_integration.py | db.py (統合) | 7 |
| test_embeddings.py | embeddings.py | 9 |
| test_embeddings_integration.py | embeddings.py (heavy) | 3 |
| test_llm.py | llm.py | 9 |
| test_chunking.py | chunking.py | 32 |
| test_reranker.py | reranker.py | 18 |
| test_prompting.py | prompting.py | 6 |
| test_ingest.py | ingest.py | 23 |
| test_ask.py | ask.py | 8 |
| test_graph.py | graph.py | 19 |
| test_metrics.py | metrics.py | 38 |
...
```text
1. get_container()  →  AppContainer 取得
...
2. get_container() → get_graph(container=...) で LangGraph ワークフローを構築
3. graph.invoke() で 2ノードパイプラインを実行:
   a. [retrieve]  TwoStageRetrieval.retrieve() を実行
      - vectorstore で SEARCH_K=10 件をベクトル検索
      - reranker で RERANK_TOP_K=3 件にリランキング
   b. [generate]  日本語プロンプト組立 → LLM生成
...
2. get_container() → get_graph(container=...) でグラフ構築
3. 各質問に対して:
   a. graph.invoke()  →  検索 + リランキング + LLM回答生成
   b. retrieval_at_k  →  正解ソースが検索結果に含まれるか
   c. faithfulness  →  キーワード含有率
   d. exact_match  →  全キーワードの含有判定
   e. measure_latency  →  処理時間計測
4. 全質問の集計結果をレポート出力
```
