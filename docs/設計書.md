...
data/csv/ ─┘                                              │
                                                          ↓
                ask.py ←── TwoStageRetrieval ←── ベクトル検索 (k=20, score_threshold=0.5)
...
1. **第1段階（Vector Search）**: pgvector のコサイン距離検索で候補を SEARCH_K 件（デフォルト20件）取得し、SCORE_THRESHOLD（デフォルト0.5）でフィルタリング
...
|---|---|---|---|
| retrieve | TwoStageRetrieval で検索+スコア閾値フィルタ+リランキングを実行（SEARCH_K件 → 閾値フィルタ → RERANK_TOP_K件） | query | reranked_documents |
| generate | 日本語プロンプトを組み立て LLM で回答生成。検索結果が空の場合は LLM を呼び出さず「該当する情報が見つかりませんでした。」を返却。ソースは重複排除 | reranked_documents, query | contexts, prompt, answer, sources |
...
| CHUNK_SIZE | 350（環境変数またはYAMLで変更可） | チャンク分割サイズ（文字数） |
| CHUNK_OVERLAP | 80（環境変数またはYAMLで変更可） | チャンク間オーバーラップ（文字数） |
| SEARCH_K | 20（環境変数またはYAMLで変更可） | ベクトル検索の取得件数 |
| RERANK_TOP_K | 3（環境変数またはYAMLで変更可） | Re-ranking後の採用件数 |
| SCORE_THRESHOLD | 0.5（環境変数またはYAMLで変更可） | ベクトル検索のスコア閾値（コサイン距離） |
...
| Protocol | メソッド | 説明 |
|---|---|---|
| VectorStoreProtocol | `similarity_search_with_score(query: str, k: int) -> list` | スコア付きベクトル検索 |
...
| クラス名 | 説明 |
|---|---|
| RagSettings | `@dataclass(frozen=True)`。`search_k`, `rerank_top_k`, `score_threshold` を保持 |
...
| retrieval_strategy | RetrievalStrategyProtocol | `TwoStageRetrieval(vectorstore, reranker, search_k, rerank_top_k, score_threshold)` |
...
| クラス名 | 説明 |
|---|---|
| TwoStageRetrieval | `@dataclass(frozen=True)`。`vectorstore`, `reranker`, `search_k`, `rerank_top_k`, `score_threshold` を保持。`retrieve(query)` でベクトル検索 → スコア閾値フィルタ → リランキングを実行 |

**処理フロー:**

1. `vectorstore.similarity_search_with_score(query, k=search_k)` でスコア付きベクトル検索（SEARCH_K件）
2. `score_threshold` 以下のスコアのドキュメントのみをフィルタリング（コサイン距離: 低いほど類似）
3. フィルタ後のドキュメントが空の場合は空リストを即座に返却（reranker を呼ばない）
4. `reranker.compress_documents(docs, query)` でリランキング
5. 上位 `rerank_top_k` 件を返却
...
Cross-Encoder によるスコアリングで検索結果を再順位付けする。`CrossEncoderReranker` クラスが `RerankerProtocol` を実装する。

**クラス・関数:**

| 名前 | 引数 | 戻り値 | 説明 |
|---|---|---|---|
| CrossEncoderReranker | model_name, top_n | — | `HuggingFaceCrossEncoder` を使った reranker クラス。`compress_documents(documents, query)` でドキュメントをスコアリングし上位 `top_n` 件を返却 |
| create_reranker | top_n (デフォルト RERANK_TOP_K) | CrossEncoderReranker | Re-rankerインスタンスを生成して返却 |
...
| load_csvs | data/csv/ 配下の全CSVファイルを行単位で読み込み、カテゴリ列を除外したテキストに変換して返す |
| main | `get_container()` → `delete_collection()` で既存コレクション削除 → vectorstore 再生成 → PDF/CSV読み込み → PDFは `split_by_structure` で分割、CSVは1行=1ドキュメント → メタデータ付与 → `container.vectorstore.add_documents()` で格納 |
...
| CSV | 1行=1ドキュメント（分割なし） | FAQ・商品情報は1行が1つの意味単位であり、分割すると情報が欠損する |
...
```text
以下の情報のみを基に、質問に簡潔に回答してください。情報に含まれていない場合は「該当する情報が見つかりませんでした」と回答してください。追加の質問や回答は生成しないでください。
...
| context_relevance | documents, expected_keywords | float (0.0〜1.0) | 検索結果ドキュメント内のキーワード出現率。検索品質の指標 |
| retrieval_mrr | documents, expected_source | float (0.0〜1.0) | MRR（Mean Reciprocal Rank）。期待ソースの出現順位の逆数 |
...
| evaluate_single | 1問分の全体評価を実行。`graph.invoke()` で回答生成 → 4指標（retrieval_hit, faithfulness, exact_match, latency）を算出 |
...
| evaluate_single_retrieval | 1問分のRetrieval単体評価を実行。LLMを呼ばず `container.retrieval_strategy.retrieve()` のみ使用。retrieval_hit, context_relevance, mrr, retrieved_count, latency を算出 |
| run_retrieval_evaluation | 全質問に対して evaluate_single_retrieval を実行し、結果リストを返す |
| print_retrieval_report | Retrieval評価レポートを出力（Retrieval@k, Context Relevance, MRR, Avg Retrieved, Latency） |
| main | `get_container()` → `get_graph(container=...)` → 全問評価 → レポート出力 |
| main_retrieval | `get_container()` → Retrieval単体評価 → レポート出力（`--retrieval-only` フラグで実行） |
...
**Retrieval単体評価レポート出力フォーマット（`make evaluate-retrieval`）:**

```text
=== Retrieval Evaluation Report ===

Chunk=350 overlap=80
Top-k=20 score_threshold=0.5
Re-rank=ON (top_k=3)

Retrieval@k: xx.x%
Context Relevance: xx.x%
MRR: x.xxx
Avg Retrieved: x.x docs
Latency: xx.xs
...
| 1行=1ドキュメント | CSV | 分割なし（1行がそのまま1ドキュメント） | ingest.py 内で直接処理 |

### 6.2 パラメータ

| パラメータ | デフォルト値 | 設定元 | 説明 |
|---|---|---|---|
| chunk_size | 350文字 | 環境変数 `CHUNK_SIZE` または setting.yaml | 1チャンクの最大文字数 |
| chunk_overlap | 80文字 | 環境変数 `CHUNK_OVERLAP` または setting.yaml | チャンク間の重複文字数 |
...
| CHUNK_SIZE | 350 | int | チャンク分割サイズ（文字数） |
| CHUNK_OVERLAP | 80 | int | チャンク間オーバーラップ（文字数） |
| SEARCH_K | 20 | int | ベクトル検索の取得件数 |
| RERANK_TOP_K | 3 | int | Re-ranking後の採用件数 |
| SCORE_THRESHOLD | 0.5 | float | ベクトル検索のスコア閾値（コサイン距離） |
...
| make evaluate-retrieval | Retrieval単体評価を実行（`python -m rag.evaluation.evaluate --retrieval-only`） |
...
| test_metrics.py | rag.evaluation.metrics | 49 |
| test_evaluate.py | rag.evaluation.evaluate | 56 |
| **合計** | **17ファイル** | **274** |
...
2. container.vectorstore.delete_collection()  →  既存コレクション削除（冪等な再取り込み）
3. create_vectorstore(container.embeddings)  →  vectorstore 再生成
4. load_pdfs()  →  data/pdf/*.pdf をページ単位で読み込み（rag.data.ingest）
5. load_csvs()  →  data/csv/*.csv を行単位で読み込み、カテゴリ列除外（rag.data.ingest）
6. PDF: split_by_structure  →  段落ベースでチャンク分割（rag.data.chunking）
7. CSV: 1行=1ドキュメント（分割なし）
8. 各チャンクに source, chunk_index メタデータを付与
9. container.vectorstore.add_documents()  →  一括格納（埋め込み生成はPGVector内部で実行）
```
...
3. graph.invoke() で 2ノードパイプラインを実行:
   a. [retrieve]  TwoStageRetrieval.retrieve() を実行（rag.pipeline.retrieval）
      - vectorstore で SEARCH_K=20 件をスコア付きベクトル検索
      - SCORE_THRESHOLD=0.5 でフィルタリング
      - reranker で RERANK_TOP_K=3 件にリランキング
   b. [generate]  検索結果が空の場合は「該当する情報が見つかりませんでした。」を即返却。
      それ以外: 日本語プロンプト組立（rag.components.prompting） → LLM生成（rag.components.llm）
4. 回答テキストと重複排除済みソースIDを標準出力に表示
```
