...
2. **第2段階（Re-ranking）**: Cross-Encoder で候補をスコアリングし直し、上位 RERANK_TOP_K 件（デフォルト5件）に絞り込み
...
| RERANK_TOP_K | 5（環境変数またはYAMLで変更可） | Re-ranking後の採用件数 |
...
| RERANK_TOP_K | 5 | int | Re-ranking後の採用件数 |
...
3. graph.invoke() で 2ノードパイプラインを実行:
   a. [retrieve]  TwoStageRetrieval.retrieve() を実行（rag.pipeline.retrieval）
      ...
      - reranker で RERANK_TOP_K=5 件にリランキング
